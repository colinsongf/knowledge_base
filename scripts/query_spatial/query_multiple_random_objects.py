...
super_value = npy.zeros(shape=(discrete_space_x,discrete_space_y))
	...
	for k in range(1,5):
		for i in range(0,discrete_space_x): 		
			for j in range(0,discrete_space_y):						
				...
				x = lookup_value_add(sample, k)			
				value_function[i][j]=x
		super_value[:][:] = super_value[:][:] + value_function[:][:]
...
ax.plot_surface(X,Y,super_value,cmap=plt.cm.jet, cstride=1, rstride=1)
plt.show()
