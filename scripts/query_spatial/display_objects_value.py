...
def random_obj_positions():
	...
	for i in range(0,number_objects):
		object_poses[i][0] = random.randrange(-4,3)
 		object_poses[i][1] = random.randrange(-2,6)		
...
ax.plot_surface(X,Y,object_location_function,cmap=plt.cm.jet, cstride=1, rstride=1)
# plt.show()

fig1 = plt.figure(1)
ax1 = fig1.add_subplot(111,projection='3d')
# X,Y=npy.meshgrid(space_dist,space_dist)
ax1.plot_surface(X,Y,value_function,cmap=plt.cm.jet, cstride=1, rstride=1)
plt.show()

# super_value_function = npy.zeros(shape=(5,discrete_space_x,discrete_space_y))
# for i in range(0,5): 
	# calculate_value_function(i)
	# super_value_function[i] = value_function

# with file('obj_value_functions.txt','w') as outfile:	
	# npy.savetxt(outfile,super_value_function,fmt='%-7.2f')

# with file('object_value_functions.txt','w') as outfile:
# 	for data_slice in super_value_function:
